...
class User extends Authenticatable implements MustVerifyEmail
{
    ...
    public function avatarHtml($size = '40px', $text = '1.25rem')
    {
        ...
        if (str_contains($avatarStr, 'avatars/')) {
            ...
            $html = '<img src="'.e($url).'" class="avatar-img" style="width:'.$size.'; height:'.$size.'; border-radius:50%; object-fit:cover; border:2px solid white; box-shadow:0 1px 2px rgba(0,0,0,0.1); cursor:pointer;">';
        } else {
            $avatars = [
            ...
            if (array_key_exists($avatarStr, $avatars)) {
                $html = $avatars[$avatarStr];
            } else {
                $initial = strtoupper(substr($this->name ?? 'U', 0, 1));
                $html = '<div class="avatar-img" style="width:'.$size.'; height:'.$size.'; border-radius:50%; background:#e2e8f0; display:flex; align-items:center; justify-content:center; font-size:'.$text.'; color:#64748b; font-weight:bold; border: 2px solid white; box-shadow: 0 1px 2px rgba(0,0,0,0.1); cursor:pointer;">'.e($initial).'</div>';
            }
        }
